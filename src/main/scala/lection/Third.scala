...
object ScalaClasses extends App {
  //java:
  //class Person {   <3 <3 <3 <3 javist odobriet
  //    String name;
  //    int age;
  //    void displayInfo(){
  ...
  class Point(val x: Int, val y: Int) { //or x:int eq private val x: Int        ydobno
    def distance(other: Point): Double = {
      import scala.math.sqrt // import ydobno
  ...
  println(s"distance between $p1 and  $p2 eq ${p1.distance(p2)}")
...
object ScalaClassesWithVar extends App {
  ...
  // Не понял в чем прикол и чем отличается
...
object ScalaCaseClasses extends App  { // Пока что удобно
...
object ScalaTraits extends App  {
  ...
  // class1 extends class2 || trait1 with trait2 with traitN
}
